    ...
    return parser = {
        ...
        parse: function (str, callback) {
            ...
            for(var j=0; j<filesLength; j++) {
                files[j]['filename'] = output.replace(ext, options.fileSuffix + (filesLength - j) + ext);
            }

            if (filesLength > 0 && options.imports) {
                for(var k=1; k<=filesLength; k++) {
                    var outputFilename = filename + options.fileSuffix + k + ext;
...
bless.Parser.cleanup = function (options, output, callback) {
    var error = null,
        ...
        filename = path.basename(output, ext),
        fileRegex = filename + options.fileSuffix + '(\\d+)' + ext;
